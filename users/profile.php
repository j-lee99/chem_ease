...
$stmt = $conn->prepare("SELECT full_name, email, profile_image FROM users WHERE id = ? AND is_deleted = 0");
...
<head>
    ...
    <style>
        ...
        @media (max-width: 767px) {
            .settings-container { padding: 1.5rem 0.9rem; }
            .settings-title { font-size: 2.1rem; }
            .profile-info { flex-direction: column; text-align: center; gap: 1.5rem; }
            .profile-avatar, .default-avatar { width: 110px; height: 110px; font-size: 2.6rem; }
            .danger-item { flex-direction: column; align-items: stretch; gap: 1.2rem; }
            .danger-item button { width: 100%; }
            .modal-dialog { margin: 0.5rem; }
            .modal-lg { max-width: 96%; }
            .otp-input { font-size: 2.5rem; letter-spacing: 0.9rem; height: 80px; }
        }
        @media (max-width: 576px) {
            .settings-title { font-size: 1.8rem; }
            .profile-avatar, .default-avatar { width: 100px; height: 100px; font-size: 2.4rem; }
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editProfileForm" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="editFullName" value="<?php echo htmlspecialchars($full_name); ?>" required>
                    </div>
                    <div class="mb-3">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" style="background-color: var(--primary-blue); border-color: var(--primary-blue);" onclick="updateProfile()">
                    Save Changes
                </button>
            </div>
        </div>
...
function calculatePasswordStrength(password) {
    ...
    if (score <= 2) return {level:'weak', text:'Weak', color:'#dc3545', width:'33%'};
    if (score <= 4) return {level:'medium', text:'Medium', color:'#ffc107', width:'66%'};
...
async function updateProfile() {
    ...
    try {
        ...
        const res = await fetch('../partial/update_profile.php', { method: 'POST', body: formData });
        ...
        if (data.email_change_pending) {
            ...
            // Live update UI
            document.getElementById('displayFullName').textContent = data.updated_name || fullName;
            document.getElementById('displayEmail').textContent = data.updated_email || email;
